import { A as DOMAdapter, G as TextureStyle, Q as v8_0_0, R as warn, V as Texture, Z as deprecation, it as Matrix, lt as eventemitter3_default, nt as Rectangle, rt as groupD8, tt as uid, z as Color } from "./Geometry-DpgSrUJG.js";
import { t as ImageSource } from "./ImageSource-8ELTfnPR.js";
import { t as Cache } from "./Cache-DFdV2AJ3.js";
import { M as FillGradient, a as FillPattern, i as toStrokeStyle, r as toFillStyle, t as GraphicsContext } from "./GraphicsContext-1a4VDAQU.js";
import { t as CanvasPool } from "./CanvasPool-1Vlz6scn.js";
...
function getPlainText(text, style) {
	...
	return parseTaggedText(text, style).map((run) => run.text).join("");
}
//#endregion
...
function measureTaggedText(text, style, wordWrap, context, measureTextFn, measureFontFn, canBreakCharsFn, wordWrapSplitFn) {
	...
	const wrappedRunsByLine = wordWrap ? wordWrapTaggedLines(runsByLine, style, context, measureTextFn, canBreakCharsFn, wordWrapSplitFn) : runsByLine;
	const lineWidths = [];
	const lineAscents = [];
	const lineDescents = [];
	...
	const width = (wordWrap && style.align !== "left" ? Math.max(maxLineWidth, style.wordWrapWidth) : maxLineWidth) + strokeWidth + (style.dropShadow ? style.dropShadow.distance : 0);
	let baseHeight = 0;
...
var contextSettings$1 = { willReadFrequently: true };
...
var _CanvasTextMetrics = class _CanvasTextMetrics {
	/**
	...
	*/
	static measureText(text = " ", style, canvas = _CanvasTextMetrics._canvas, wordWrap2 = style.wordWrap) {
		...
		if (hasTagStyles(style) && hasTagMarkup(text)) {
			const result = measureTaggedText(text, style, wordWrap2, _CanvasTextMetrics._context, _CanvasTextMetrics._measureText, _CanvasTextMetrics.measureFont, _CanvasTextMetrics.canBreakChars, _CanvasTextMetrics.wordWrapSplit);
		...
		const measurements = new _CanvasTextMetrics(text, style, width, _CanvasTextMetrics._adjustHeightForStyle(baseHeight, style), lines, lineWidths, lineHeight + style.leading, maxLineWidth, fontProperties);
	...
	*/
	static _getAlignWidth(maxLineWidth, style, wordWrapEnabled) {
		return wordWrapEnabled && style.align !== "left" ? Math.max(maxLineWidth, style.wordWrapWidth) : maxLineWidth;
	}
	/**
	...
	*/
	static _measureText(text, letterSpacing, context) {
		...
		if (metricWidth > 0) if (useExperimentalLetterSpacing) {
	...
	*/
	static _wordWrap(text, style, canvas = _CanvasTextMetrics._canvas) {
		return wordWrap(text, style, canvas, _CanvasTextMetrics._measureText, _CanvasTextMetrics.canBreakWords, _CanvasTextMetrics.canBreakChars, _CanvasTextMetrics.wordWrapSplit);
	}
	/**
...
var _TextStyle = class _TextStyle extends eventemitter3_default {
	constructor(style = {}) {
		super();
		/**
...
var _DynamicBitmapFont = class _DynamicBitmapFont extends AbstractBitmapFont {
	/**
	...
	*/
	constructor(options) {
		...
		*/
		...
		if (dynamicOptions.textureStyle) this._textureStyle = dynamicOptions.textureStyle instanceof TextureStyle ? dynamicOptions.textureStyle : new TextureStyle(dynamicOptions.textureStyle);
	...
	*/
	...
	_applyKerning(newChars, context, fontScale) {
		...
		for (let i = 0; i < newChars.length; i++) {
			...
			for (let j = 0; j < this._currentChars.length; j++) {
				...
				if (amount && this.chars[first]) this.chars[first].kerning[second] = amount / fontScale;
				total = context.measureText(first + second).width;
				amount = total - (c1 + c2);
				if (amount && this.chars[second]) this.chars[second].kerning[first] = amount / fontScale;
			}
		}
	}
	_nextPage() {
		...
		if (this._textureStyle) texture.source.style = this._textureStyle;
		const pageData = {
			canvasAndContext,
			texture
		};
	...
	_setupContext(context, style, resolution) {
		...
		if (style._fill) context.fillStyle = getCanvasFillStyle(style._fill, context);
		if (style.dropShadow) {
	...
	_drawGlyph(context, metrics, x, y, fontScale, style) {
		...
		const strokeThickness = (style._stroke?.width ?? 0) * fontScale;
...
function getBitmapTextLayout(chars, style, font, trimEnd) {
	...
	if (shouldCollapseSpaces || shouldCollapseNewlines) {
		...
		for (let c = 0; c < chars.length; c++) {
			...
			if (isBreakingSpace(char)) if (shouldCollapseSpaces && isCollapsibleSpace(char)) {
	...
	const nextWord = (word) => {
		...
		if (currentWord.index > 0 || !shouldCollapseSpaces) firstWord = false;
		currentWord.width = 0;
		currentWord.index = 0;
	...
	for (let i = 0; i < chars.length + 1; i++) {
		...
		if (/(?:\s)/.test(char) || char === "\r" || char === "\n" || isEnd) {
			if (!firstWord && style.wordWrap && checkIsOverflow(currentLine.width + currentWord.width)) {
				nextLine();
			...
			if (breakWords && checkIsOverflow(currentWord.width + nextCharWidth)) {
				if (!firstWord) nextLine();
				nextWord(currentWord);
				nextLine();
			}
			...
			if (isBreakAfterChar(char)) {
				if (!firstWord && style.wordWrap && checkIsOverflow(currentLine.width + currentWord.width)) nextLine();
				nextWord(currentWord);
			}
		}
		previousChar = char;
	...
	nextLine();
	if (style.wordWrap && style.align !== "left") layoutData.width = Math.max(layoutData.width, adjustedWordWrapWidth);
...
function alignJustify(measurementData) {
	...
	for (let i = 0; i < measurementData.lines.length - 2; i++) {
		...
		const spaceWidth = (width - line.width) / totalSpaces;
...
var BitmapFontManagerClass = class {
	...
	*/
	getFont(text, style) {
		...
		if (!Cache.has(fontFamilyKey)) {
			...
			fontCount++;
			if (fontCount > 50) warn("BitmapText", `You have dynamically created ${fontCount} bitmap fonts, this can be inefficient. Try pre installing your font styles using \`BitmapFont.install({name:"style1", style})\``);
			fnt.once("destroy", () => {
				fontCount--;
	...
	*/
	getLayout(text, style, trimEnd = true) {
		...
		if (this.measureCache.has(id)) return this.measureCache.get(id);
		const layoutData = getBitmapTextLayout(CanvasTextMetrics.graphemeSegmenter(text), style, bitmapFont, trimEnd);
	...
	*/
	...
	install(...args) {
		...
		const font = new DynamicBitmapFont({
			style,
			overrideFill: options.dynamicFill ?? this._canUseTintForStyle(style),
...
//#endregion
//#region node_modules/pixi.js/lib/scene/text-bitmap/BitmapFont.mjs
var BitmapFont = class extends AbstractBitmapFont {
	constructor(options, url) {
		super();
		const { textures, data } = options;
		Object.keys(data.pages).forEach((key) => {
			const texture = textures[data.pages[parseInt(key, 10)].id];
		...
		Object.keys(data.chars).forEach((key) => {
			...
			const texture = new Texture({
				frame: groupD8.transformRectCoords(charData, textureFrame, textureRotate, new Rectangle()),
...
//#endregion
export { isCollapsibleSpace as A, NEWLINE_MATCH_REGEX as C, getCharacterGroups as D, collapseSpaces as E, hasTagMarkup as F, hasTagStyles as I, parseTaggedText as L, tokenize as M, trimRight as N, isBreakAfterChar as O, getPlainText as P, NEWLINES_SET as S, collapseNewlines as T, BREAK_AFTER_CHARS as _, DynamicBitmapFont as a, COLLAPSIBLE_SPACES_SET as b, getCanvasFillStyle as c, wordWrap as d, measureTaggedText as f, BREAKING_SPACES_SET as g, BREAKING_SPACES as h, getBitmapTextLayout as i, isNewline as j, isBreakingSpace as k, fontStringFromTextStyle as l, wordWrapTaggedLines as m, BitmapFontManager as n, AbstractBitmapFont as o, tokenizeTaggedRuns as p, resolveCharacters as r, TextStyle as s, BitmapFont as t, CanvasTextMetrics as u, BREAK_AFTER_CHARS_SET as v, NEWLINE_SPLIT_REGEX as w, NEWLINES as x, COLLAPSIBLE_SPACES as y };

//# sourceMappingURL=BitmapFont-ChWZOE6M.js.map