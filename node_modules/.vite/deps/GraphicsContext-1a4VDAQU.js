import { $ as v8_3_4, A as DOMAdapter, B as Bounds, K as definedProps, M as InstructionSet, N as multiplyHexColors, P as BigPool, Q as v8_0_0, R as warn, V as Texture, Z as deprecation, at as Point, dt as extensions, it as Matrix, lt as eventemitter3_default, mt as __toESM, nt as Rectangle, pt as __commonJSMin, tt as uid, ut as ExtensionType, z as Color } from "./Geometry-DpgSrUJG.js";
import { t as ImageSource } from "./ImageSource-8ELTfnPR.js";
import { R as earcut, n as DefaultBatcher, t as GCManagedHash } from "./GCManagedHash-DapmiZVU.js";
...
var import_parse_svg_path = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	for (let i = 0; i < commands.length; i++) {
		...
		switch (type) {
			...
			case "z":
				...
				currentSubPath = null;
				break;
			default: warn(`Unknown SVG path command: ${type}`);
		}
		if (type !== "Z" && type !== "z") {
...
var Circle = class Circle {
	...
	*/
	strokeContains(x, y, width, alignment = .5) {
		if (this.radius === 0) return false;
		const dx = this.x - x;
		const dy = this.y - y;
...
//#endregion
//#region node_modules/pixi.js/lib/maths/shapes/Ellipse.mjs
var Ellipse = class Ellipse {
	/**
...
var Polygon = class Polygon {
	...
	*/
	containsPolygon(polygon) {
		...
		if (!thisBounds.containsRect(otherBounds)) return false;
	...
	*/
	strokeContains(x, y, strokeWidth, alignment = .5) {
		...
		for (let i = 0; i < iterationLength; i += 2) {
			...
			if (squaredDistanceToLineSegment(x, y, x1, y1, x2, y2) <= (Math.sign((x2 - x1) * (y - y1) - (y2 - y1) * (x - x1)) < 0 ? leftWidthSquared : rightWidthSquared)) return true;
		}
		return false;
	}
	/**
...
//#endregion
//#region node_modules/pixi.js/lib/maths/shapes/RoundedRectangle.mjs
...
var RoundedRectangle = class RoundedRectangle {
	/**
	...
	*/
	strokeContains(pX, pY, strokeWidth, alignment = .5) {
...
var BatchableGraphics = class {
	...
	get blendMode() {
		if (this.renderable && this.applyTransform) return this.renderable.groupBlendMode;
		return "normal";
	}
	get color() {
		...
		if (renderable) return multiplyHexColors(bgr, renderable.groupColor) + (this.alpha * renderable.groupAlpha * 255 << 24);
...
//#endregion
//#region node_modules/pixi.js/lib/scene/graphics/shared/buildCommands/buildCircle.mjs
var buildCircle = {
	extension: {
...
//#endregion
//#region node_modules/pixi.js/lib/scene/graphics/shared/const.mjs
...
//#endregion
//#region node_modules/pixi.js/lib/scene/graphics/shared/buildCommands/buildRectangle.mjs
var buildRectangle = {
	extension: {
		type: ExtensionType.ShapeBuilder,
		name: "rectangle"
...
//#endregion
//#region node_modules/pixi.js/lib/scene/graphics/shared/buildCommands/buildTriangle.mjs
var buildTriangle = {
	extension: {
		type: ExtensionType.ShapeBuilder,
		name: "triangle"
...
var _FillGradient = class _FillGradient {
	constructor(...args) {
		/**
		...
		*/
		...
		options = {
			...options.type === "radial" ? _FillGradient.defaultRadialOptions : _FillGradient.defaultLinearOptions,
	...
	*/
	buildLinearGradient() {
		...
		this.texture = new Texture({ source: new ImageSource({
			resource: canvas,
		...
		m.rotate(angle);
		m.translate(x0, y0);
		if (this.textureSpace === "local") m.scale(defaultSize, defaultSize);
		this.transform = m;
	}
	/**
	* Builds the internal texture and transform for the gradient.
	...
	*/
	buildGradient() {
		...
		if (this.type === "linear") this.buildLinearGradient();
		else this.buildRadialGradient();
	}
	/**
...
var tempTextureMatrix$1 = new Matrix();
...
function generateTextureMatrix(out, style, shape, matrix) {
	...
	if (style.textureSpace === "local") {
		...
		if (style.width) bounds.pad(style.width);
	...
	if (matrix) textureMatrix.append(tempTextureMatrix$1.copyFrom(matrix).invert());
...
function buildContextBatches(context, gpuContext) {
	...
	for (let i = 0; i < context.instructions.length; i++) {
		...
		if (instruction.action === "texture") addTextureToGeometryData(instruction.data, batches, geometryData);
			...
			if (isStroke && hole) addShapePathToGeometryData(hole.shapePath, style, true, batches, geometryData);
			if (hole) shapePath.shapePrimitives[shapePath.shapePrimitives.length - 1].holes = hole.shapePath.shapePrimitives;
...
function addTextureToGeometryData(data, batches, geometryData) {
	...
	if (!build.build(rect, points)) return;
...
function addShapePathToGeometryData(shapePath, style, isStroke, batches, geometryData) {
	...
	shapePath.shapePrimitives.forEach(({ shape, transform: matrix, holes }) => {
		...
		if (!build.build(shape, points)) return;
		const indexOffset = indices.length;
		const vertOffset = vertices.length / 2;
		let topology = "triangle-list";
		if (matrix) transformVertices(points, matrix);
		if (!isStroke) if (holes) {
			const holeIndices = [];
			...
			if (!lineStyle.pixelLine) buildLine(points, lineStyle, false, close, vertices, indices);
			else {
...
function getHoleArrays(holePrimitives) {
	...
	for (let k = 0; k < holePrimitives.length; k++) {
		...
		const holePoints = [];
		if (shapeBuilders[holePrimitive.type].build(holePrimitive, holePoints)) holeArrays.push(holePoints);
	}
	return holeArrays;
}
//#endregion
...
var GpuGraphicsContext = class {
	...
	reset() {
		if (this.batches) this.batches.forEach((batch) => {
			BigPool.return(batch);
		});
		if (this.graphicsData) BigPool.return(this.graphicsData);
...
var _GraphicsContextSystem = class _GraphicsContextSystem {
	constructor(renderer) {
		this._renderer = renderer;
		this._managedContexts = new GCManagedHash({
			renderer,
			type: "resource",
	...
	*/
	updateGpuContext(context) {
		...
		if (context.dirty || !hasContext) {
			if (hasContext) gpuContext.reset();
			...
			else gpuContext.isBatchable = true;
			context.dirty = false;
		}
		return gpuContext;
	}
	...
	*/
	...
	_initContextRenderData(context) {
		const graphicsData = BigPool.get(GraphicsContextRenderData, { maxTextures: this._renderer.limits.maxBatchableTextures });
...
_GraphicsContextSystem.defaultOptions = { bezierSmoothness: .5 };
...
var curveAngleToleranceEpsilon$1 = .01;
var mAngleTolerance$1 = 0;
var mCuspLimit = 0;
function buildAdaptiveBezier(points, sX, sY, cp1x, cp1y, cp2x, cp2y, eX, eY, smoothness) {
	let distanceTolerance = (PATH_DISTANCE_EPSILON$1 - Math.min(.99, Math.max(0, smoothness ?? GraphicsContextSystem.defaultOptions.bezierSmoothness))) / 1;
...
var curveAngleToleranceEpsilon = .01;
var mAngleTolerance = 0;
function buildAdaptiveQuadratic(points, sX, sY, cp1x, cp1y, eX, eY, smoothness) {
	let distanceTolerance = (PATH_DISTANCE_EPSILON - Math.min(.99, Math.max(0, smoothness ?? GraphicsContextSystem.defaultOptions.bezierSmoothness))) / 1;
	distanceTolerance *= distanceTolerance;
...
function buildArcTo(points, x1, y1, x2, y2, radius) {
	...
	const a1 = points[points.length - 1] - y1;
	const b1 = fromX - x1;
	const a2 = y2 - y1;
	const b2 = x2 - x1;
...
function approxUnitArc(ang1, ang2) {
	const a1 = ang2 === -1.5707963267948966 ? -.551915024494 : 4 / 3 * Math.tan(ang2 / 4);
	const a = ang2 === 1.5707963267948966 ? .551915024494 : a1;
...
function roundedShapeArc(g, points, radius) {
	...
	for (let i = 0; i < points.length; i++) {
		...
		if (v1.nx * v2.nx - v1.ny * -v2.ny < 0) if (angle < 0) angle = Math.PI + angle;
		else {
			angle = Math.PI - angle;
			radDirection = -1;
			drawDirection = true;
...
function roundedShapeQuadraticCurve(g, points, radius, smoothness) {
	...
	for (let i = 0; i < numPoints; i++) {
		...
		else start = pointLerp(thisPoint, lastPoint, Math.min(lastEdgeLength / 2, pRadius) / lastEdgeLength);
		const nextEdgeLength = distance(nextPoint, thisPoint);
		let end;
		if (nextEdgeLength < 1e-4) end = thisPoint;
		else end = pointLerp(thisPoint, nextPoint, Math.min(nextEdgeLength / 2, pRadius) / nextEdgeLength);
...
var ShapePath = class {
	...
	*/
	roundPoly(x, y, radius, sides, corner, rotation = 0, smoothness) {
		...
		const sideLength = radius * Math.sin(Math.PI / sides) - .001;
	...
	*/
	roundShape(points, radius, useQuadratic = false, smoothness) {
		...
		if (useQuadratic) roundedShapeQuadraticCurve(this, points, radius, smoothness);
		else roundedShapeArc(this, points, radius);
		return this.closePath();
	}
	/**
...
var GraphicsPath = class GraphicsPath {
	/**
	...
	*/
	constructor(instructions, signed = false) {
		...
		if (typeof instructions === "string") parseSVGPath(instructions, this);
	...
	*/
	get shapePath() {
		if (!this._shapePath) this._shapePath = new ShapePath(this);
		if (this._dirty) {
			this._dirty = false;
...
function parseLinearGradient(child) {
	const gradient = new FillGradient(parseSVGFloatAttribute(child, "x1", 0), parseSVGFloatAttribute(child, "y1", 0), parseSVGFloatAttribute(child, "x2", 1), parseSVGFloatAttribute(child, "y2", 0), (child.getAttribute("gradientUnits") || "objectBoundingBox") === "objectBoundingBox" ? "local" : "global");
...
function parseSVGStyle(svg, session) {
	...
	for (const key in styleAttributes) {
		...
		if (attribute) parseAttribute(session, result, key, attribute.trim());
	}
	if (style) {
		const styleParts = style.split(";");
		for (let i = 0; i < styleParts.length; i++) {
			const [key, value] = styleParts[i].trim().split(":");
			if (styleAttributes[key]) parseAttribute(session, result, key, value.trim());
		}
	}
	return {
	...
	switch (id) {
		case "stroke":
			if (value !== "none") {
				...
				} else result.strokeStyle.color = Color.shared.setValue(value).toNumber();
		...
		case "fill":
			if (value !== "none") {
				...
				} else result.fillStyle.color = Color.shared.setValue(value).toNumber();
				result.useFill = true;
			}
			break;
		case "fill-opacity":
...
function checkForNestedPattern(subpathsWithArea) {
	if (subpathsWithArea.length <= 2) return true;
	...
	if (largestToSecondRatio > 3 && secondToSmallestRatio < 2) return false;
	return true;
}
function getFillInstructionData(context, index = 0) {
	const instruction = context.instructions[index];
	if (!instruction || instruction.action !== "fill") throw new Error(`Expected fill instruction at index ${index}, got ${instruction?.action || "undefined"}`);
	return instruction.data;
}
//#endregion
//#region node_modules/pixi.js/lib/scene/graphics/shared/svg/utils/pathOperations.mjs
function extractSubpaths(pathData) {
	return pathData.split(/(?=[Mm])/).filter((part) => part.trim().length > 0);
}
function calculatePathArea(pathData) {
	...
	const xs = [];
	const ys = [];
	for (let i = 0; i < numbers.length; i += 2) if (i + 1 < numbers.length) {
		xs.push(numbers[i]);
		ys.push(numbers[i + 1]);
	}
...
function appendSVGPath(pathData, graphicsPath) {
	...
	for (const instruction of tempPath.instructions) graphicsPath.instructions.push(instruction);
}
//#endregion
...
function renderChildren(svg, session, fillStyle, strokeStyle) {
	...
	switch (svg.nodeName.toLowerCase()) {
		case "path": {
			...
			if (hasExplicitEvenodd && hasMultipleSubpaths) {
				const subpathsWithArea = subpaths.map((subpath) => ({
					path: subpath,
					area: calculatePathArea(subpath)
				}));
				subpathsWithArea.sort((a, b) => b.area - a.area);
				if (subpaths.length > 3 || !checkForNestedPattern(subpathsWithArea)) for (let i = 0; i < subpathsWithArea.length; i++) {
					...
					} else session.context.cut();
				}
				else for (let i = 0; i < subpathsWithArea.length; i++) {
					const subpath = subpathsWithArea[i];
					const isHole = i % 2 === 1;
					session.context.beginPath();
					const newPath = new GraphicsPath(void 0, true);
				...
				graphicsPath = new GraphicsPath(d, fillRule ? fillRule === "evenodd" : true);
...
function getMaxMiterRatio(path, miterLimit) {
	...
	for (let i = 0; i < shapePrimitives.length; i++) {
		...
		for (let j = 0; j < n; j += 2) {
			...
			let clampedCos = (dx0 * dx1 + dy0 * dy1) / Math.sqrt(len0Sq * len1Sq);
			if (clampedCos < -1) clampedCos = -1;
			else if (clampedCos > 1) clampedCos = 1;
			const sinHalfAngle = Math.sqrt((1 - clampedCos) * .5);
...
var _GraphicsContext = class _GraphicsContext extends eventemitter3_default {
	constructor() {
		super(...arguments);
		/** @internal */
	...
	*/
	...
	fill(style, alpha) {
		...
		if (this._tick === 0 && lastInstruction?.action === "stroke") path = lastInstruction.data.path;
	...
	*/
	stroke(style) {
		let path;
		const lastInstruction = this.instructions[this.instructions.length - 1];
		if (this._tick === 0 && lastInstruction?.action === "fill") path = lastInstruction.data.path;
		else path = this._activePath.clone();
		if (!path) return this;
		if (style != null) this._strokeStyle = toStrokeStyle(style, _GraphicsContext.defaultStrokeStyle);
		this.instructions.push({
			action: "stroke",
			data: {
	...
	*/
	cut() {
		for (let i = 0; i < 2; i++) {
			...
			if (lastInstruction) {
				if (lastInstruction.action === "stroke" || lastInstruction.action === "fill") if (lastInstruction.data.hole) lastInstruction.data.hole.addPath(holePath);
				else {
	...
	get bounds() {
		...
		for (let i = 0; i < this.instructions.length; i++) {
			...
			if (action === "stroke") {
				...
				if (data.style.join === "miter") outerPadding *= getMaxMiterRatio(data.path, data.style.miterLimit);
	...
	*/
	destroy(options = false) {
		...
		if (typeof options === "boolean" ? options : options?.texture) {
			...
			if (this._fillStyle.texture) this._fillStyle.fill && "uid" in this._fillStyle.fill ? this._fillStyle.fill.destroy() : this._fillStyle.texture.destroy(destroyTextureSource);
			if (this._strokeStyle.texture) this._strokeStyle.fill && "uid" in this._strokeStyle.fill ? this._strokeStyle.fill.destroy() : this._strokeStyle.texture.destroy(destroyTextureSource);
		}
...
//#endregion
export { Circle as $, shapeBuilders as A, closePointEps as B, buildArc as C, GraphicsContextRenderData as D, GpuGraphicsContext as E, buildPolygon as F, BatchableGraphics as G, buildCircle as H, triangulateWithHoles as I, buildUvs as J, transformVertices as K, buildPixelLine as L, FillGradient as M, buildTriangle as N, GraphicsContextSystem as O, buildRectangle as P, Ellipse as Q, buildLine as R, buildArcTo as S, buildAdaptiveBezier as T, buildEllipse as U, curveEps as V, buildRoundedRectangle as W, Polygon as X, RoundedRectangle as Y, squaredDistanceToLineSegment as Z, GraphicsPath as _, FillPattern as a, roundedShapeQuadraticCurve as b, calculatePathArea as c, getFillInstructionData as d, parseSVGPath as et, parseSVGStyle as f, parseSVGFloatAttribute as g, parseSVGDefinitions as h, toStrokeStyle as i, generateTextureMatrix as j, buildContextBatches as k, extractSubpaths as l, extractSvgUrlId as m, getMaxMiterRatio as n, SVGParser as o, styleAttributes as p, buildSimpleUvs as q, toFillStyle as r, appendSVGPath as s, GraphicsContext as t, checkForNestedPattern as u, ShapePath as v, buildAdaptiveQuadratic as w, buildArcToSvg as x, roundedShapeArc as y, getOrientationOfPoints as z };

//# sourceMappingURL=GraphicsContext-1a4VDAQU.js.map