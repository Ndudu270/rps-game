import { A as DOMAdapter, M as InstructionSet, N as multiplyHexColors, P as BigPool, Q as v8_0_0, R as warn, V as Texture, Z as deprecation, dt as extensions, it as Matrix, rt as groupD8, s as RendererType, ut as ExtensionType, z as Color } from "./Geometry-DpgSrUJG.js";
import { c as ViewContainer, n as State, o as CanvasSource, w as bgr2rgb } from "./Filter-CLwi5nka.js";
import { A as SpritePipe, I as AlphaMaskPipe, L as BatcherPipe, N as CustomRenderPipe, Y as AbstractRenderer, a as SharedSystems, j as RenderGroupPipe, k as BlendModePipe, t as RenderTargetSystem, z as color32BitToUniform } from "./RenderTargetSystem-BQ-jHc9C.js";
import { t as GCManagedHash } from "./GCManagedHash-DapmiZVU.js";
import { A as shapeBuilders, G as BatchableGraphics, M as FillGradient, O as GraphicsContextSystem, R as buildLine, a as FillPattern, j as generateTextureMatrix, t as GraphicsContext } from "./GraphicsContext-1a4VDAQU.js";
...
var _CanvasGraphicsContextSystem = class _CanvasGraphicsContextSystem {
...
_CanvasGraphicsContextSystem.defaultOptions = { bezierSmoothness: .5 };
var CanvasGraphicsContextSystem = _CanvasGraphicsContextSystem;
//#endregion
//#region node_modules/pixi.js/lib/scene/graphics/canvas/CanvasGraphicsPipe.mjs
...
var GraphicsPipe = class {
	...
	execute(graphics) {
		...
		const context = graphics.context;
		if (!renderer.graphicsContext.getGpuContext(context).batches.length) return;
...
//#endregion
//#region node_modules/pixi.js/lib/scene/graphics/init.mjs
...
var Graphics = class Graphics extends ViewContainer {
	/**
	* Creates a new Graphics object.
	* @param options - Options for the Graphics.
	*/
	constructor(options) {
		if (options instanceof GraphicsContext) options = { context: options };
	...
	*/
	endFill() {
		...
		if (strokeStyle.width !== GraphicsContext.defaultStrokeStyle.width || strokeStyle.color !== GraphicsContext.defaultStrokeStyle.color || strokeStyle.alpha !== GraphicsContext.defaultStrokeStyle.alpha) this.context.stroke();
		return this;
	}
	/**
	* @param {...any} args
...
function canUseNewCanvasBlendModes() {
	...
	try {
		...
		const context = DOMAdapter.get().createCanvas(6, 1).getContext("2d");
...
//#endregion
//#region node_modules/pixi.js/lib/rendering/renderers/canvas/utils/canvasUtils.mjs
var canvasUtils = {
	canvas: null,
	...
	getCanvasSource: (texture) => {
		...
		if (isPMA) {
			if (resource instanceof HTMLCanvasElement || typeof OffscreenCanvas !== "undefined" && resource instanceof OffscreenCanvas) {
				if (!needsResize) return resource;
			}
			const cached = canvasUtils._unpremultipliedCache.get(source);
			if (cached?.resourceId === source._resourceId) return cached.canvas;
		}
		if (resource instanceof Uint8Array || resource instanceof Uint8ClampedArray || resource instanceof Int8Array || resource instanceof Uint16Array || resource instanceof Int16Array || resource instanceof Uint32Array || resource instanceof Int32Array || resource instanceof Float32Array || resource instanceof ArrayBuffer) {
			...
			if (source.format === "bgra8unorm") for (let i = 0; i < data.length && i + 3 < bytes.length; i += 4) {
				data[i] = bytes[i + 2];
				data[i + 1] = bytes[i + 1];
				data[i + 2] = bytes[i];
				data[i + 3] = bytes[i + 3];
			}
			else data.set(bytes.subarray(0, data.length));
			context.putImageData(imageData, 0, 0);
			canvasUtils._canvasSourceCache.set(source, {
				canvas,
		...
		if (isPMA) {
			...
			for (let i = 0; i < data.length; i += 4) {
				...
				if (a > 0) {
					...
					data[i] = Math.min(255, data[i] * alphaInv + .5);
					data[i + 1] = Math.min(255, data[i + 1] * alphaInv + .5);
					data[i + 2] = Math.min(255, data[i + 2] * alphaInv + .5);
				}
			}
			context.putImageData(imageData, 0, 0);
			canvasUtils._unpremultipliedCache.set(source, {
				canvas,
				resourceId: source._resourceId
			});
			return canvas;
	...
	getTintedCanvas: (sprite, color) => {
		...
		if (cachedCanvas?.tintId === resourceId) return cachedCanvas;
	...
	getTintedPattern: (texture, color) => {
		...
		if (pattern?.tintId === resourceId) return pattern;
		if (!canvasUtils.canvas) canvasUtils.canvas = DOMAdapter.get().createCanvas();
	...
	applyPatternTransform: (pattern, matrix, invert = true) => {
		...
		const domMatrix = new DOMMatrixCtor([
			matrix.a,
			matrix.b,
			matrix.c,
			matrix.d,
	...
	tintWithMultiply: (texture, color, canvas) => {
		...
		if (rotate) canvasUtils._applyInverseRotation(context, rotate, crop.width, crop.height);
...
var _CanvasBatchAdaptor = class _CanvasBatchAdaptor {
	...
	execute(batchPipe, batch) {
		...
		for (let i = 0; i < elements.length; i++) {
			...
			const tint = bgr2rgb(multiplyHexColors(argb & 16777215, globalTint));
			...
			const applyRotateTransform = rotate && !(!needsRepeat && (tint !== 16777215 || rotate));
			if (applyRotateTransform) {
				_CanvasBatchAdaptor._tempPatternMatrix.copyFrom(quad.transform);
				groupD8.matrixAppendRotationInv(_CanvasBatchAdaptor._tempPatternMatrix, rotate, dx, dy, dw, dh);
				contextSystem.setContextTransform(_CanvasBatchAdaptor._tempPatternMatrix, quad.roundPixels === 1, void 0, isFromCachedRenderGroup && isRootTarget);
			...
			if (needsRepeat) {
				...
				if (canTint && fitsFrame) patternSource = canvasUtils.getTintedCanvas({ texture }, tint);
				...
				_CanvasBatchAdaptor._tempPatternMatrix.set(a * pixelWidth, b * pixelHeight, c * pixelWidth, d * pixelHeight, tx * pixelWidth, ty * pixelHeight);
				...
				const processedSource = tint !== 16777215 || rotate ? canvasUtils.getTintedCanvas({ texture }, tint) : source;
				const isProcessed = processedSource !== source;
				context.drawImage(processedSource, isProcessed ? 0 : sx, isProcessed ? 0 : sy, isProcessed ? processedSource.width : sw, isProcessed ? processedSource.height : sh, drawX, drawY, drawW, drawH);
			}
		}
	}
};
...
_CanvasBatchAdaptor.extension = {
	type: [ExtensionType.CanvasPipesAdaptor],
	name: "batch"
};
...
function buildShapePath$1(context, shape) {
	switch (shape.type) {
		...
		default: {
			...
			if (poly.closePath) context.closePath();
			break;
		}
	}
}
function addHolePaths$1(context, holes) {
	...
	for (let i = 0; i < holes.length; i++) {
		...
		if (hasTransform) context.restore();
	}
	return true;
}
var CanvasStencilMaskPipe = class {
	...
	execute(instruction) {
		if (instruction.action !== "pushMaskBegin" && instruction.action !== "popMaskEnd") return;
		const canvasRenderer = this._renderer;
		const contextSystem = canvasRenderer.canvasContext;
		const context = contextSystem?.activeContext;
		if (!context) return;
		if (instruction.action === "popMaskEnd") {
			if (this._canvasMaskStack.pop()) context.restore();
			return;
		}
		if (instruction.inverse) this._warnOnce("inverse", "CanvasRenderer: inverse masks are not supported on Canvas2D; ignoring inverse flag.");
		const maskContainer = instruction.mask.mask;
		if (!(maskContainer instanceof Graphics)) {
			this._warnOnce("nonGraphics", "CanvasRenderer: only Graphics masks are supported in Canvas2D; skipping mask.");
		...
		context.save();
		contextSystem.setContextTransform(graphics.groupTransform, (canvasRenderer._roundPixels | graphics._roundPixels) === 1);
		context.beginPath();
		let drewPath = false;
		let hasHoles = false;
		for (let i = 0; i < instructions.length; i++) {
			...
			const shapePath = instructionData.data?.path?.shapePath;
		...
		if (hasHoles) context.clip("evenodd");
		else context.clip();
...
var CanvasContextSystem = class {
	...
	*/
	...
	init() {
		...
		this.rootContext = this._renderer.canvas.getContext("2d", { alpha });
	...
	setBlendMode(blendMode) {
		...
		if (!mappedBlend) {
			if (!this._warnedBlendModes.has(blendMode)) {
				console.warn(`CanvasRenderer: blend mode "${blendMode}" is not supported in Canvas2D; falling back to "source-over".`);
...
function buildShapePath(context, shape) {
	switch (shape.type) {
		...
		case "ellipse": {
			const ellipse = shape;
			if (context.ellipse) context.ellipse(ellipse.x, ellipse.y, ellipse.halfWidth, ellipse.halfHeight, 0, 0, Math.PI * 2);
			else {
				context.save();
				context.translate(ellipse.x, ellipse.y);
				context.scale(ellipse.halfWidth, ellipse.halfHeight);
...
function getCanvasStyle(style, tint, textureMatrix, currentTransform) {
	...
	if (fill instanceof FillGradient) {
		...
		if (gradientTexture) {
			...
			if (currentTransform && !style.textureSpace) patternMatrix.append(currentTransform);
	...
	if (texture && texture !== Texture.WHITE) {
		if (!texture.source.resource) return emptyCanvasStyle;
...
var CanvasGraphicsAdaptor = class {
	...
	execute(graphicsPipe, renderable) {
		...
		const globalTint = globalColor & 16777215;
		const groupTint = bgr2rgb(multiplyHexColors(groupColorAlpha & 16777215, globalTint));
		...
		for (let i = 0; i < instructions.length; i++) {
			...
			if (instruction.action === "texture") {
				...
				if (tint2 !== 16777215) drawSource = canvasUtils.getTintedCanvas({ texture }, tint2);
				const frame = texture.frame;
				const resolution = texture.source._resolution ?? texture.source.resolution ?? 1;
				...
				if (hasTransform || rotate) {
					...
					if (hasTransform) tempMatrix.append(transform);
					if (rotate) groupD8.matrixAppendRotationInv(tempMatrix, rotate, data2.dx, data2.dy, data2.dw, data2.dh);
					contextSystem.setContextTransform(tempMatrix, roundPixels === 1);
				} else contextSystem.setContextTransform(baseTransform, roundPixels === 1);
				context.drawImage(drawSource, sx, sy, drawSource === source ? sw : drawSource.width, drawSource === source ? sh : drawSource.height, rotate ? 0 : data2.dx, rotate ? 0 : data2.dy, data2.dw, data2.dh);
				if (hasTransform || rotate) contextSystem.setContextTransform(baseTransform, roundPixels === 1);
				continue;
			}
			...
			for (let j = 0; j < shapePrimitives.length; j++) {
				...
				const canvasStyle = getCanvasStyle(style, tint, hasTexture ? generateTextureMatrix(tempTextureMatrix, style, primitive.shape, textureTransform) : null, hasTransform ? tempGradientMatrix.copyFrom(baseTransform).append(transform) : baseTransform);
				if (hasTransform) {
					context.save();
					context.transform(transform.a, transform.b, transform.c, transform.d, transform.tx, transform.ty);
				}
				if (isStroke) {
					const strokeStyle = style;
					if (strokeStyle.alignment !== .5 && !strokeStyle.pixelLine) {
						const points = [];
						const vertices = [];
						const indices = [];
						if (shapeBuilders[primitive.shape.type]?.build(primitive.shape, points)) {
							buildLine(points, strokeStyle, false, primitive.shape.closePath ?? true, vertices, indices);
					...
					context.beginPath();
					buildShapePath(context, primitive.shape);
					if (addHolePaths(context, primitive.holes)) context.fill("evenodd");
					else context.fill();
				}
				if (hasTransform) context.restore();
			}
...
var CanvasRenderTargetAdaptor = class {
	...
	*/
	startRenderPass(renderTarget, clear, clearColor, viewport) {
		...
		if (clear) this.clear(renderTarget, clear, clearColor, viewport);
	}
	/**
	...
	*/
	clear(renderTarget, _clear, clearColor, viewport) {
		const context = this._renderTargetSystem.getGpuRenderTarget(renderTarget).context;
		const bounds = viewport || {
			x: 0,
			y: 0,
			width: renderTarget.pixelWidth,
			height: renderTarget.pixelHeight
		};
	...
	*/
	copyToTexture(sourceRenderSurfaceTexture, destinationTexture, originSrc, size, originDest) {
		const sourceCanvas = this._renderTargetSystem.getGpuRenderTarget(sourceRenderSurfaceTexture).canvas;
		...
		context.drawImage(sourceCanvas, originSrc.x, originSrc.y, size.width, size.height, dx, dy, size.width, size.height);
		destSource.update();
...
//#endregion
//#region node_modules/pixi.js/lib/rendering/renderers/canvas/renderTarget/CanvasRenderTargetSystem.mjs
...
var CanvasTextureSystem = class {
	...
	*/
	getPixels(texture) {
		...
		return {
			pixels: canvas.getContext("2d", { willReadFrequently: true }).getImageData(0, 0, canvas.width, canvas.height).data,
			width: canvas.width,
			height: canvas.height
		};
	}
...
var DefaultCanvasAdapters = [CanvasBatchAdaptor, CanvasGraphicsAdaptor];
var systems = [];
var renderPipes = [];
...
//#endregion
export { CanvasGraphicsPipe as _, CanvasGraphicsAdaptor as a, mapCanvasBlendModesToPixi as c, CanvasBatchAdaptor as d, canvasUtils as f, GraphicsPipe as g, GraphicsGpuData as h, CanvasRenderTargetAdaptor as i, CanvasStencilMaskPipe as l, Graphics as m, CanvasTextureSystem as n, CanvasLimitsSystem as o, canUseNewCanvasBlendModes as p, CanvasRenderTargetSystem as r, CanvasContextSystem as s, CanvasRenderer as t, CanvasColorMaskPipe as u, CanvasGraphicsContextSystem as v };

//# sourceMappingURL=CanvasRenderer-CSAe0PLE.js.map